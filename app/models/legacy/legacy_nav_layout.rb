class LegacyNavLayout < LegacyData
  ...
  def confirm_placement
    ...
    if section_id and !section_id.zero?
      ...
      page.descendents.each do |p| 
        unless p.placements.find_by_view_type "#{nav_side}_nav"
          p.placements.create :child_item_id => imported.id, :child_item_type => 'Page', :layout_area => nav_side, :view_type => "#{nav_side}_nav"
        end
      end
    end
    ...
    if class_id and !class_id.zero?
      ...
      return unless page
      page.placements.create :child_item_type => 'Page', :child_item_id => imported.id, :layout_area => nav_side, :view_type => "#{nav_side}_nav"
    end
    ...
    if introtext_id  == AMP_INTROTEXTS['content']
      ...
      Page.all(:conditions => ["legacy_type not like ?", "%nav_layout%" ] ).each do |page|
        ...
        page.placements.create :child_item_id => imported.id, :child_item_type => 'Page', :layout_area => nav_side, :view_type => "#{nav_side}_nav"
      end
    end
